...
# ╔═╡ 0cd88834-6a15-11ec-3c35-8592307036b2
using Turing, Distributions, MCMCChains,StatsPlots,CairoMakie,CSV, Colors,DataFrames, DataFramesMeta, LinearAlgebra, PDMats

# ╔═╡ b08bdecd-5690-43fe-8422-d0f7a5bd9fb2


# ╔═╡ 6fedf54f-2e52-49ec-9f9f-d3740b70915b
set_theme!(Theme(Axis=(;xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ e5a9faf7-2221-4e24-a690-2e7667f2dcd3
d_raw = CSV.read("../input/data-mvn.txt", DataFrame)

# ╔═╡ fe20473d-b0cd-4748-b84a-d05f0fe001bd
CairoMakie.scatter(d_raw.Y1, d_raw.Y2)

# ╔═╡ 749c1862-bfde-4ecf-9edb-6423b52fa435
Y = Matrix(d_raw)

# ╔═╡ db111d76-c9ee-42f5-8ae1-033877f979ab
@model function multinormal(Y)
	n_samples = size(Y,1)
	...
	#r ~ Uniform(-1, 1)
	#Σ = Symmetric([σ1*σ1 r*σ1*σ2; r*σ1*σ2 σ2*σ2])

	# Turing does not support LKJCholesky as for ver. 0.1.x
	# https://github.com/TuringLang/TuringGLM.jl/issues/21
	#Σ ~ LKJCholesky(2, 1.0)

	Σ ~ InverseWishart(2 + 1, Matrix{Float64}(I, 2, 2))
...
# ╔═╡ f79aaaaf-3d38-4e7e-a0ac-687b44a3127e
model = multinormal(Y)

# ╔═╡ 016b2a16-c873-4caa-b89c-aade4ebb6708
chain = sample(model, NUTS(), 1_000)

# ╔═╡ 134f4a28-9917-490b-baac-625d1af65138
describe(chain)

# ╔═╡ e418c229-0b4a-47a7-88ff-9b5c8088b7da
StatsPlots.plot(chain)

# ╔═╡ c756676b-97c3-485c-b6b1-29e121d37662
