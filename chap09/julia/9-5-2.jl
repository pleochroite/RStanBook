...
# ╔═╡ 0182f070-4bcf-4aa2-a633-2c1be6b66b69
using Turing, Distributions, MCMCChains, StatsPlots, CairoMakie, CSV, DataFrames, DataFramesMeta, Colors

# ╔═╡ c11738fc-6e04-11ec-159f-b555709e11bd
md"""
### 9.5.2 欠測値
"""

# ╔═╡ f7de6737-56bd-43d6-b67d-6868dff52b57
set_theme!(Theme(Axis=(;xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 3970e62d-1a95-4009-a646-c15bcee0ee23


# ╔═╡ 5e0c3ea7-f484-43ba-a914-e15ee4d7549e
dat = CSV.read("../input/data-conc-2-NA-wide.txt", DataFrame)

# ╔═╡ a154716a-e791-4e91-b73d-f7f443ef934d
function recode_data(val)
	if val == "NA"
		missing
	else
		parse(Float64, val)
...
# ╔═╡ 6f820c49-e82e-4a96-b796-e817219f2193
function parse_time(val)
	val[5:end]
end

# ╔═╡ 7535c7af-6cbb-4456-a1f0-2c4a28850e8d
dat_long = DataFrames.stack(dat, Not(:PersonID))

# ╔═╡ b53f48c4-eb45-46f5-bcb2-843d0954fc35
d = @transform(dat_long, 
			:Time = parse.(Int, parse_time.(:variable)), 
			:value = recode_data.(:value))

# ╔═╡ 51579ec9-3b35-4810-a909-6306b78618d3
begin
	X = d[:,[:PersonID, :Time]]
	Y = d[:, :value]
end

# ╔═╡ b8732c92-1019-46c0-bbf5-d6e0466829fb
let
	fig = Figure()
	axs = [Axis(fig[i,j], xlabel="time", ylabel="y", 
			title="PersonID: $(i +(j-1)*4)", 
			limits=(0, 25, 0, 40))
			for j ∈ 1:4, i ∈ 1:4]
	...
	for j ∈ 1:4
		for i ∈ 1:4
			k = i + (j-1) * 4
			_tmpdf = @subset(d, :PersonID .== k)
			CairoMakie.scatterlines!(axs[i,j], _tmpdf.Time, _tmpdf.value)
...
# ╔═╡ 4021470d-735b-4d63-9797-7b1545faac4b
@model function nonlinear(X, Y)
	...
	for i ∈ 1:size(X,1)
		Y[i] ~ Normal(a[X[i,:PersonID]] * (1 - exp(-b[X[i,:PersonID]] * X[i,:Time])), σy)
	end
	return a, b
end

# ╔═╡ 6271566b-5e7c-459e-ad26-b6b135ddf36c
model = nonlinear(X, Y)

# ╔═╡ 551e81e5-c79b-4ff0-8c5d-b0fa3d291b85
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_500, 4)

# ╔═╡ d4b4dcf1-024d-4b9e-9916-98c6e6b7669f
describe(chain)

# ╔═╡ bba3a825-a607-4c11-a328-9176d017b216
StatsPlots.plot(chain[:,[:a0, :b0, :σa, :σb, :σy],:])

# ╔═╡ c315f007-8dd6-4e7b-b579-f7c6550d91de
begin
	gqs = generated_quantities(model, chain)
	a = reduce(hcat, [gqs[s,1][1] for s ∈ 1:1500])'
	b = reduce(hcat, [gqs[s,1][2] for s ∈ 1:1500])'
end


# ╔═╡ 2b5672aa-5de0-4460-b91f-979de1177158
let
	qs = [0.05, 0.5, 0.95]
	aqs = [quantile.([a[:, i + (j-1)*4] for i ∈ 1:4, j ∈ 1:4], q) 
		for q ∈ qs]
	bqs = [quantile.([b[:, i + (j-1)*4] for i ∈ 1:4, j ∈ 1:4], q)
		for q ∈ qs]
	...
	for i ∈ 1:4, j ∈ 1:4
		...
		_tmpdf = @subset(d, :PersonID .== k)
		CairoMakie.scatterlines!(ax[k], _tmpdf.Time, _tmpdf.value)


		f50(x) = aqs[2][i,j] * (1 - exp(-bqs[2][i,j] * x))
		CairoMakie.lines!(ax[k], xrange, f50.(xrange), color=:red)

		f_low(x) = aqs[1][i,j] * (1 - exp(-bqs[1][i,j] * x))
		f_high(x) = aqs[3][i,j] * (1 - exp(-bqs[3][i,j] * x))
		CairoMakie.band!(ax[k], xrange, f_low.(xrange), f_high.(xrange),
			color=RGBA(0.5, 0.5, 0, 0.3))
