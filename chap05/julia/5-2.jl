...
# ╔═╡ bb714a36-5ffa-11ec-1aaa-5f4b92bea909
using Turing, Distributions, MCMCChains, CairoMakie, StatsPlots, CSV, DataFrames, DataFramesMeta

# ╔═╡ 635d078c-70f9-4452-bc43-1c5fec394da8
md"""
## 5.2 二項ロジスティック回帰
"""

# ╔═╡ 7eac2c0f-d5cf-4fcf-b67e-a013b02b6175
set_theme!(Theme(Axis=(xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 1c1f0a79-4be5-44db-80fb-71f6c9120cec
d_raw = CSV.read("../input/data-attendance-2.txt", DataFrame)


# ╔═╡ 26dd028f-0769-4dd7-96bf-f5c8780ec44d
d = @transform(d_raw, :Score = :Score ./ 200)

# ╔═╡ 2b872f40-b82e-4392-b321-37916ae3d403
begin
	X = d[:,Not([:PersonID, :Y])]
	Y = d[:, :Y]
end

# ╔═╡ afbd1846-6a64-4dbb-8804-5cf8c6b17a4c
md"""
### 5.2.2 データの分布の確認
"""

# ╔═╡ cf151e9c-9022-4f81-b256-e7a869f67eb3
StatsPlots.corrplot(Array(d[:, Not(:PersonID)]))

# ╔═╡ 64463a18-f64d-4fb8-ba1e-97e3dd8e5f09
md"""
### 5.2.5' Turingで実装
"""

# ╔═╡ 49b05a7a-b439-46ee-87a8-ba4d6b2c0d84
sig(x) = 1 / (1 + exp(-x))

# ╔═╡ f2ce5898-27b2-49d2-b770-42c524e04d4a
@model function binomial_logistic_regression(X, Y)
	b1 ~ Normal(0, 100)
	b2 ~ Normal(0, 100)
	b3 ~ Normal(0, 100)
	...
	for n in 1:size(X,1)
		...
		Y[n] ~ Binomial(X[n,:M], q[n])
	end
end

# ╔═╡ 556f8bd7-8ec3-451c-b530-6cd5bc6361d9
model = binomial_logistic_regression(X, Y)

# ╔═╡ f33ec373-f06f-47ed-b9b8-4a78e379d39d
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_000, 3)

# ╔═╡ 1cf7d3a0-8b6c-49ec-9624-e27a43052768
describe(chain)

# ╔═╡ 32c3a19c-9b10-436e-8c81-2c08725ea90d
StatsPlots.plot(chain)

# ╔═╡ 266dec9b-fa45-4d34-b54b-fd25f22010c2
md"""
### 5.2.7 図によるモデルのチェック
"""

# ╔═╡ 7f3affd3-697e-423c-853f-7d01723b4e61
let
	_X = hcat(ones(50), X[:,[1,2]])
	_Xmat = Matrix(_X)
	_Xm = X[:,:M]

	keys = [:b1, :b2, :b3]
	ws = DataFrame([quantile(chain[:, key, 1], [0.2, 0.5, 0.8])
		for key in keys], keys)
	
	# 予測値は M[n] * q[n]になる
	q20 = sig.(_Xmat * Vector(ws[1,:])) .* _Xm
	q50 = sig.(_Xmat * Vector(ws[2,:])) .* _Xm
	q80 = sig.(_Xmat * Vector(ws[3,:])) .* _Xm
	...
	CairoMakie.scatter!(ax, reshape(Y, size(Y,1)), q50)
	CairoMakie.errorbars!(ax, reshape(Y, size(Y,1)), q50, lowerrors, higherrors)
	CairoMakie.lines!(ax, [10,70], [10,70]; linestyle=:dot, color=:gray20)

	fig
end

# ╔═╡ a2aa7528-e595-4241-9942-ea3193ae7347


# ╔═╡ c760487e-70d5-4c77-a52e-1354088b318c
