...
# ╔═╡ a867fa38-6076-11ec-0f55-77372ce79ac2
using Turing, Distributions, MCMCChains, CairoMakie, StatsPlots, CSV, DataFrames, DataFramesMeta

# ╔═╡ 45080e36-51b6-4d06-bc7b-04b233a81abc
md"""
## 5.4 ポアソン回帰
"""

# ╔═╡ 54cb7eae-8cf4-4324-b0af-67636d80b68b
set_theme!(Theme(Axis=(xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 8601a490-1a4b-4941-a7ed-729ea1afa0e7
d_raw = CSV.read("../input/data-attendance-2.txt", DataFrame)

# ╔═╡ c4840c15-570e-4550-8fc5-79a2e91096b6
d = @transform(d_raw, :Score = :Score ./ 200)

# ╔═╡ 7b50902e-5154-4c71-acd3-67f32fa951ac
begin
	X = d[:,Not([:PersonID, :M, :Y])]
	M = d[:, :M]
	Y = d[:, :Y]
end

# ╔═╡ ddb5db2b-6a87-45f1-a2f0-de3f43340977
md"""
### 5.4.3' Turingで実装
"""

# ╔═╡ bc9d5216-0145-4391-8c1f-9c378d795d5d
@model function poisson_regression(X, Y)
	...
	for n in 1:size(X,1)
		λ = exp(b1 + b2 * X[n,:A] + b3 * X[n,:Score])
		Y[n] ~ Poisson(λ)
	end
end

# ╔═╡ 7cb84d8c-1907-4903-b0e0-c698197fe25b
model = poisson_regression(X, Y)

# ╔═╡ 227de45d-d97e-4a19-9894-0a21a4407e49
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_000, 4)

# ╔═╡ 37f48726-da3a-4be9-938c-4e03f00d615f
describe(chain)

# ╔═╡ 9dc0c877-4c06-4b10-ba77-45633cc7052a
StatsPlots.plot(chain)

# ╔═╡ 36e3734e-6c4b-46aa-a324-c932f7be01d3
md"""
## 練習問題 (5)
### 図を使ったモデルの検証
"""

# ╔═╡ c387a5ec-a16e-4280-84a3-b1e1dab06e11
begin
	keys = [:b1, :b2, :b3]
	B = DataFrame([quantile(chain[:,key,1], [0.2, 0.5, 0.8]) for key in keys], keys)
end

# ╔═╡ 1619503b-100a-4bf9-9929-c14296ec40a2
let
	pred_low = exp.(Matrix(hcat(ones(size(X,1)), X)) * Vector(B[1,:]))
	pred_mid = exp.(Matrix(hcat(ones(size(X,1)), X)) * Vector(B[2,:]))
	pred_high = exp.(Matrix(hcat(ones(size(X,1)), X)) * Vector(B[3,:]))

	lerror = pred_mid .- pred_low
	herror = pred_high .- pred_mid

	fig = Figure()
	ax = Axis(fig[1,1], xlabel="observed", ylabel="predicted")

	CairoMakie.scatter!(ax, Y, pred_mid)
	CairoMakie.errorbars!(ax, Y, pred_mid, lerror, herror)
	CairoMakie.lines!(ax, [15, 60], [15, 60], linestyle=:dot, color=:green)
