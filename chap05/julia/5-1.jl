...
# ╔═╡ 1cb13f37-4a00-42d3-9337-6e1a462a86e7
using Turing, Distributions, MCMCChains, CairoMakie, StatsPlots, CSV, DataFrames, DataFramesMeta

# ╔═╡ 94a2b386-5fb9-11ec-201e-4bd1509d364f
md"""
## 5.1 重回帰
"""

# ╔═╡ a861a339-2924-49fc-8ec7-c1e20feb9bf4
...
d_raw = CSV.read("../input/data-attendance-1.txt", DataFrame)
...
# ╔═╡ f9e06c0c-85a0-4dac-ab78-6ed65963969c
StatsPlots.plot(chain)

# ╔═╡ cfd2cdd4-df39-471b-b4b0-c9f11ffb1203
md"""
### 5.1.8 図によるモデルのチェック
"""

# ╔═╡ 9c07e9f9-4f06-440e-b6ec-1d35e05462dc
let
	fig = Figure()
	ax = Axis(fig[1,1], xlabel="Score", ylabel="Y")

	xs = range(50, 200, length=150)
		
	# visualize raw data
	for val in unique(d_raw.A)
		tmpdf = subset(d_raw, :A => ByRow(==(val)))
		CairoMakie.scatter!(ax, tmpdf.Score, tmpdf.Y, label="$(val)")

	# visualize MCMC samples
		for i in 1:5:size(chain[:,:,1], 1)
			w1 = chain[i,:b1,1]
			w2 = chain[i,:b2,1]	
			w3 = chain[i,:b3,1]
			f(A, X) = w1 + w2 * A + w3 * X / 200
			CairoMakie.lines!(ax, xs, f.(val, xs), color=RGBA(0, 0.5, 0, 0.05))
		end
	end
...
# ╔═╡ 9517dee0-a56a-4a71-b6ed-dcbf3a31b307
md"""
実測値と予測値のプロット
"""

# ╔═╡ 04ead709-3505-463f-bd71-638227c56559
let
	coef_symbols = [:b1, :b2, :b3]
	ws = DataFrame([quantile(chain[:,key,1], [0.2, 0.5, 0.8]) 
		for key in coef_symbols], coef_symbols)
	
	_X = Matrix(hcat(ones(50), X))
	q20 = _X * Vector(ws[1,:])
	q50 = _X * Vector(ws[2,:])
	q80 = _X * Vector(ws[3,:])
	...
	CairoMakie.scatter!(ax, q50, reshape(Y, 50), color=d.A)
	CairoMakie.errorbars!(ax, q50, reshape(Y, 50), lowerrors, higherrors)
	CairoMakie.lines!(ax, [0,0.5], [0,0.5]; linestyle=:dot, color=:gray20)

	fig
end

# ╔═╡ 16d09b7a-27a7-492a-9249-67d579d5385a
md"""
推定されたノイズの分布
"""

# ╔═╡ 0cf66f33-4f9f-4630-aecc-1a1a75e05312
let
	tmpdf = DataFrame(chain[:,[:b1, :b2, :b3], 1])
	w = Matrix(tmpdf[:,[3, 4, 5]])
	...
	CairoMakie.hist!(ax, reshape(mean(w * _X' .- Y', dims=1), 50))

	xs = range(-0.15, 0.15, length=100)
	μ, σ = 0, mean(chain[:,:σ, :])
	CairoMakie.lines!(ax, xs, pdf.(Normal(μ,sqrt(σ)), xs); linestyle=:dot)
