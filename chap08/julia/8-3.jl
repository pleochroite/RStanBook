...
# ╔═╡ 0ae04858-6599-11ec-0f69-e1f0bddbc248
using Turing, MCMCChains, StatsPlots, CairoMakie, Distributions, CSV, DataFrames, DataFramesMeta

# ╔═╡ 99cb7a8e-dc40-4100-9566-03c295f022ad
md"""
## 8.3 非線形モデルの階層モデル
"""

# ╔═╡ fc7c262f-22e9-498e-9025-7d84cc670fef
set_theme!(Theme(Axis=(;xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 74e58c98-457a-4859-8162-50ede8e85042
d = CSV.read("../input/data-conc-2.txt", DataFrame)

# ╔═╡ 26d2ac5d-f7b0-419c-8d7b-ab77c629981e
Y = Array(d[:,Not(:PersonID)])

# ╔═╡ b90ea781-428a-42dd-9510-0416bc95c709
md"""
### 8.3.1 データの分布の確認
"""

# ╔═╡ efcea9d8-656d-4388-9ade-cac7d2977c11
let
	fig = Figure()
	ax = [Axis(fig[i,j], 
		limits=(0,25, 0, 35), 
		title = "$(j + (i - 1) * 4)",
		label="$(j + (i - 1) * 4)") for i ∈ 1:4, j ∈ 1:4]
	for i ∈ 1:4, j ∈ 1:4
		k = j + (i-1) * 4
		xs = [1, 2, 4, 8, 12, 24]
		CairoMakie.scatterlines!(ax[k], xs, Vector(d[k,Not(:PersonID)]))
		if i ≠ 1
			hideydecorations!(ax[k], ticks=false, grid=false)
		end
		if j ≠ 4
			hidexdecorations!(ax[k], ticks=false, grid=false)
		end
	end
	rowgap!(fig.layout, 4)
	colgap!(fig.layout, 4)
...
# ╔═╡ 67e68ac3-d3a3-4eb1-a884-edf63fc3f014
md"""
### 8.3.4' Turingで実装
"""

# ╔═╡ 6ff82fd9-04cd-4598-a579-c6e74ed46e71
@model function nonlinear(Y)
	time = [1, 2, 4, 8, 12, 24]

	a0 ~ Normal(0, 100)
	b0 ~ Normal(0, 100)
	σa ~ truncated(Normal(0, 100), 0, Inf)
	σb ~ truncated(Normal(0, 100), 0, Inf)
	σy ~ truncated(Normal(0, 100), 0, Inf)
	
	log_a ~ filldist(Normal(a0, σa), 16)
	log_b ~ filldist(Normal(b0, σb), 16)
	...
	for n ∈ 1:16
		for t ∈ 1:length(time)
			Y[n,t] ~ Normal(a[n] * (1 - exp(-b[n] * time[t])), σy)
		end
	end
	return a, b
end	

# ╔═╡ c07cd318-4000-414a-a66c-d85aca3b9da2
model = nonlinear(Y)

# ╔═╡ 4aa10534-48e4-42cd-b9b7-9626bf6e83e8
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_500, 4)

# ╔═╡ 042c464b-91f9-49aa-97dc-27e20a5b38b3
describe(chain)

# ╔═╡ ce39efc5-21b5-4aa6-80ec-c6f2891db9fd
StatsPlots.plot(chain[:,[:a0, :b0, :σa, :σb, :σy],:])

# ╔═╡ 043531a3-036e-420a-8e34-821db5fd5abf
md"""
### 図によるモデルのチェック
#### データ$y$と予測値の信頼区間
"""

# ╔═╡ 6432c4ba-0454-4c31-a991-ac311436fd65
generated_q = generated_quantities(model, chain)

# ╔═╡ 10c1db47-f140-46a3-ad68-8a909a9593c0
a = reduce(hcat, [generated_q[s,1][1] for s ∈ 1:1500])'

# ╔═╡ 7149cb1b-b7c6-4077-8b7a-b5a3f914ab6a
b = reduce(hcat, [generated_q[s,1][2] for s ∈ 1:1500])'

# ╔═╡ 345de936-612b-4ce0-8626-fadf8ba74c39
let
	qs = [0.05, 0.5, 0.95]
	aqs = [quantile.([a[:, j + (i-1)*4] for i ∈ 1:4, j ∈ 1:4], q) 
		for q ∈ qs]
	bqs = [quantile.([b[:, j + (i-1)*4] for i ∈ 1:4, j ∈ 1:4], q)
		for q ∈ qs]

	fig = Figure()
	ax = [Axis(fig[i,j], 
		limits=(0,25, 0, 35), 
		title = "PersonID: $(j + (i - 1) * 4)",
		label="$(j + (i - 1) * 4)") for i ∈ 1:4, j ∈ 1:4]

	xrange = range(0, 25, length=100)
	for i ∈ 1:4, j ∈ 1:4
		k = j + (i-1) * 4
		xs = [1, 2, 4, 8, 12, 24]
		CairoMakie.scatterlines!(ax[k], xs, Vector(d[k,Not(:PersonID)]))

		for (q, color) ∈ enumerate([:orange, :red, :orange])
			f(x) = aqs[q][i,j] * (1 - exp(-bqs[q][i,j] * x))
			CairoMakie.lines!(ax[k], xrange, f.(xrange), color=color)
		end
...
# ╔═╡ 3ae28afa-f91c-456c-a1fc-a731da9507e0
md"""
#### 予測値と実測値のプロット
"""

# ╔═╡ a03ef399-ebf4-4a25-acfb-79647ef95ec3
_dl = DataFrames.stack(d, Not(:PersonID), variable_name=:t, value_name=:y)

# ╔═╡ 0382697d-0b5a-4719-9b11-1e53c597c124
begin
	parse_time(txt) = parse(Int, txt[5:end])
	dl = @transform(_dl, :t = parse_time.(:t))
end

# ╔═╡ 3b44d940-9e18-4212-b7eb-2ecb0236d4b2
begin
	aqs = [[quantile(a[:,i], q) for i ∈ 1:16] for q ∈ [0.05, 0.5, 0.95]]
	bqs = [[quantile(b[:,i], q) for i ∈ 1:16] for q ∈ [0.05, 0.5, 0.95]]
end

# ╔═╡ 075d4b15-6f82-4147-8bdf-6986360fe19c
dpred = @transform(dl, 
	:pred05 = aqs[1][:PersonID] .* (1 .- exp.(-bqs[1][:PersonID] .* :t)),
	:pred50 = aqs[2][:PersonID] .* (1 .- exp.(-bqs[2][:PersonID] .* :t)),
	:pred95 = aqs[3][:PersonID] .* (1 .- exp.(-bqs[3][:PersonID] .* :t)))

# ╔═╡ f8a42fc0-53c9-45b3-9119-f358df6f4cbb
let
	cycle = Cycle([:color, :marker], covary=true)
	update_theme!(Scatter=(cycle=cycle,))
	
	fig = Figure()
	ax = Axis(fig[1,1], xlabel="observed", ylabel="predicted")

	for p ∈ unique(dpred[:,:PersonID])
		_tmpdf = @subset(dpred, :PersonID .== p)
		CairoMakie.scatter!(ax, _tmpdf.y, _tmpdf.pred50;
			markersize=25, label="$(p)")

		lerror = _tmpdf.pred50 .- _tmpdf.pred05
		herror = _tmpdf.pred95 .- _tmpdf.pred50
		
		CairoMakie.errorbars!(ax, _tmpdf.y, _tmpdf.pred50, 
			lerror, herror, color=:gray50)
	end
	CairoMakie.lines!(ax, [0, 35], [0, 35], linestyle=:dot)

	axislegend("PersonID"; position=:rb, nbanks=2)
	fig
end

# ╔═╡ 591ed669-ee77-44f6-a514-26e8bf7c9a85
md"""
#### 推定されたノイズの分布
"""

# ╔═╡ 7df9a8cb-173e-41f4-ae0c-840f96b33628
let
	fig = Figure()
	...
	ϵ = dpred[:,:y] .- dpred[:,:pred50]
	CairoMakie.hist!(ax1, ϵ)

	σy = mean([median(chain[:,:σy,c]) for c in 1:4])

	xs = range(-5, 5, length=50)
	CairoMakie.lines!(ax2, xs, pdf.(Normal(0, σy), xs), linestyle=:dot)
...
git-tree-sha1 = "0baa25c6164c9872815875b1bc73b6a00ae195ed"
...
git-tree-sha1 = "a11b8c73872f917518348597ab26c5d74ce18fe9"
...
git-tree-sha1 = "76fa204a77af72e4bb41e0f796bdcd20749e5a86"
