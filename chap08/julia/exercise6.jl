...
# ╔═╡ 3766e2e6-6987-11ec-26ed-99f0f4588ac7
using Turing, Distributions, MCMCChains,StatsPlots,CairoMakie,CSV, Colors,DataFrames, DataFramesMeta, GLM , Statistics, MLBase

# ╔═╡ d088a321-16cc-4965-b917-d79584ad39e1
set_theme!(Theme(Axis=(;xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 815e7263-0c99-469b-b11d-b7ce1498f5c7
d_raw = CSV.read("../input/data7a.csv", DataFrame)

# ╔═╡ a35fd27d-91e4-45b9-9f6b-cb337f03f33f
CairoMakie.hist(d_raw.y)

# ╔═╡ 89a4ce09-b2cb-42b7-b9a6-029d573c3827
sig(x) = 1 / (1 + exp(-x))

# ╔═╡ bae99c54-9b81-4a45-b3a3-c8910046ba3b
@model function ml_binomial(Y)
	num_samples = length(Y)

	b ~ Normal(0, 100)
	σ ~ truncated(Normal(0, 100), 0, Inf)
	q ~ filldist(Normal(0, σ), num_samples)
	p = Vector(undef, num_samples)
	@. p = sig(b + q)
...
# ╔═╡ 68ed8fd8-f0b1-4c06-aebd-30bf730ab72d
begin
	X = d_raw[:,:id]
	Y = d_raw[:,:y]
end

# ╔═╡ 977ffd26-5b83-4627-9813-97e36b4ff948
model = ml_binomial(Y)
...
# ╔═╡ d5142e9f-0c3f-4f56-9998-377dead7bcfa
StatsPlots.plot(chain[:,[:b, :σ],1])

# ╔═╡ 9dbcb1fc-d563-450a-ab3d-d08b9de29cb9
