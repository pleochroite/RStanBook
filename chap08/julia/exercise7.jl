...
# ╔═╡ 10189e2c-698d-11ec-2234-33227ceee76e
using Turing, Distributions, MCMCChains,StatsPlots,CairoMakie,CSV, Colors,DataFrames, DataFramesMeta, GLM , Statistics, MLBase

# ╔═╡ 297bacf7-f2c1-4f74-b535-f9d00c7c1702
set_theme!(Theme(Axis=(;xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ dfed6a4c-0f97-4f9d-8f00-f6085fa6ca2b
d_raw = CSV.read("../input/d1.csv", DataFrame)

# ╔═╡ 4e3eed88-7f5f-4201-a3cd-c765505f7250
unique(d_raw.pot)

# ╔═╡ 43ca319e-31db-4c00-8b14-0ffa21ec7a66
unique(d_raw.f)

# ╔═╡ 96cdca51-7018-483c-ba11-48c03a343523
CairoMakie.hist(d_raw.y)

# ╔═╡ 0d6f2452-46e4-4322-8969-0abca14f527f
begin
	colnames = [:potA, :potB, :potC, :potD, :potE, :potF, :potG, :potH, :potI]
	pot_ohe = DataFrame(((unique(d_raw.pot) .== permutedims(d_raw.pot))')[:,1:end-1], colnames)
end
	
	

# ╔═╡ 53874a84-df83-4063-964a-4bff0935cdc0
f_ohe = d_raw.f .== "C"

# ╔═╡ d338fe46-33b6-4a34-97f5-675d918a87e7
X = hcat(pot_ohe, f_ohe)

# ╔═╡ ebe43554-a22c-45b6-8bc2-ad3eaa777a84
rename!(X, :x1 => :f_c)

# ╔═╡ f3ce62bf-8820-48b7-8149-b921014fd3e4
Y = d_raw.y

# ╔═╡ 3d7b78cd-290d-4721-8057-0a91b95efcf0
@model function ml_pois(X, Y)
	n_samples = length(Y)

	σ ~ truncated(Normal(0, 100), 0, Inf)
	b ~ filldist(Normal(0, σ), n_samples)

	σpot ~ truncated(Normal(0, 100), 0, Inf)
	b1 ~ Normal(0, 100)
	b2 ~ filldist(Normal(0, σpot), 9)
	b3 ~ Normal(0, 100)

	λ = Vector(undef, n_samples)
	λ = exp.(b1 .+ Matrix(X[:,Not(:f_c)]) * b2 .+ b3 * X[:,:f_c] .+ b)
	for n ∈ 1:n_samples
		Y[n] ~ Poisson(λ[n])
	end
	return λ
...
# ╔═╡ 57a77630-ed4a-4a44-844e-29e04d4d60e7


# ╔═╡ 755e3cec-44ee-4722-9ee3-bb08456de316
model = ml_pois(X, Y)

# ╔═╡ d82f8453-d646-433e-9a55-700d6fba64fe
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_500, 4)

# ╔═╡ 0b1e5961-1be8-401e-8415-164b90afeeaf
describe(chain)

# ╔═╡ 3983208f-3ba1-486d-8252-d30c347372ec
StatsPlots.plot(chain[:,[:σ, :σpot, :b1, :b3, Symbol("b2[1]"), Symbol("b2[2]"), Symbol("b2[3]"), Symbol("b2[4]"), Symbol("b2[5]"), Symbol("b2[6]"), Symbol("b2[7]"), Symbol("b2[8]"), Symbol("b2[9]")],:])

# ╔═╡ dbb36214-5733-426d-a295-9033f939cf63
l = generated_quantities(model, chain)

# ╔═╡ 0839c75d-635b-4e08-a554-a0a35c341fda
ls = hcat(l[:,1]...)'

# ╔═╡ a46c451d-6b07-4e62-a121-3966c51b2ec6
begin
	q10 = [quantile(ls[:,i], 0.1) for i ∈ 1:100]
	q50 = [quantile(ls[:,i], 0.5) for i ∈ 1:100]
	q90 = [quantile(ls[:,i], 0.9) for i ∈ 1:100]
end

# ╔═╡ bd859e5a-6df7-4375-88e6-ac8b57fa06ed
let
	cycle = Cycle([:color, :marker], covary=true)
	update_theme!(Scatter=(cycle=cycle,))
	_tmpdf = @transform(d_raw, :q10 = q10, :q50 = q50, :q90 = q90)

	low_error = _tmpdf.q50 .- _tmpdf.q10
	high_error = _tmpdf.q90 .- _tmpdf.q50

	_tmpdf2 = @transform(_tmpdf, :lerror = low_error, :herror = high_error)
	
	fig = Figure()
	ax = Axis(fig[1,1], xlabel="observed", ylabel="predicted")

	for s ∈ unique(_tmpdf2.pot)
		_subdf = @subset(_tmpdf2, :pot .== s)
		CairoMakie.scatter!(ax, _subdf.y, _subdf.q50, label="$(s)", markersize=15)
		CairoMakie.errorbars!(ax, _subdf.y, _subdf.q50, _subdf.lerror, _subdf.herror)
	end

	CairoMakie.lines!(ax, [0, 45], [0, 45], linestyle=:dot, color=:green)
	axislegend("Pot"; position=:rb)
	fig
end

# ╔═╡ e85f5c53-0bc7-4ea5-b380-aaedd0a14311
