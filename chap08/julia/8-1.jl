...
# ╔═╡ a455b71d-b6b6-4825-ba68-e4e7a9456a42
using Turing, MCMCChains, StatsPlots, CairoMakie, Distributions, CSV, DataFrames, DataFramesMeta

# ╔═╡ 84206000-6e24-4ab4-bf73-e28b14bf36e8
using GLM, LinearAlgebra

# ╔═╡ a8343b20-60a3-11ec-33ff-d9b74bcc8f2f
md"""
## 8.1 階層モデルの導入
"""

# ╔═╡ e9905333-ff53-4a43-9523-16eb1abefdf6
set_theme!(Theme(Axis=(xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 79e947d7-888d-4d41-b57b-befde864b267
d = CSV.read("../input/data-salary-2.txt", DataFrame)

# ╔═╡ 9feff2f2-e887-4c40-96e0-ad283d7172d8
begin
	X = d[:,Not(:Y)]
	Y = d[:,:Y]
end

# ╔═╡ 867d761f-549e-45d5-bd68-0684f74459f7
md"""
### 8.1.1 解析の目的とデータの分布の確認
"""

# ╔═╡ 453af6ad-6798-4ce4-8285-2f8b19219f75
let
	cycle = Cycle([:color, :marker], covary=true)
	set_theme!(Scatter=(;cycle=cycle,))
	fig = Figure()
...
# ╔═╡ 5592408e-83e6-457f-b7dd-08f825ba582c
md"""
### 8.1.2 グループ差を考えない場合
"""

# ╔═╡ 3e2a3e9d-30e5-4941-88e3-07d284f50a53
glm_all = glm(@formula(Y ~ X), d, Normal(), IdentityLink())

# ╔═╡ 3e081950-0c9c-48db-9244-7c020bcc35fb
coef(glm_all)

# ╔═╡ 7d65d609-03fc-44ea-952f-2dd07b169a07
let
	cycle = Cycle([:color, :marker], covary=true)
	set_theme!(Scatter=(;cycle=cycle,))
	fig = Figure()
	...
	CairoMakie.lines!(ax, xs, f.(xs), color=RGBA(0.5,0.5,0.5,0.5), linewidth=20)	
	
	axislegend("KID"; position=:lt)
	fig
end

# ╔═╡ a07fbe2d-dd50-4552-9822-5848bdba348f
md"""
Turingで実装
"""

# ╔═╡ b300c633-6d60-4cc2-9412-9a8ebb44ac62
md"""
### 8.1.3 グループごとに切片と傾きを持つ場合
"""

# ╔═╡ 0f9f7359-01f1-4d9b-ac70-19b52e0bdc93
let
	fig = Figure()
	...
	for i in 1:2
		for j in 1:2
			...
			_model = glm(@formula(Y ~ X), _d, Normal(), IdentityLink())
			a = coef(_model)[1]
			b = coef(_model)[2]
			f(x) = a + b * x
			CairoMakie.lines!(axs[i,j], xs, f.(xs), 
				color=RGBA(0,1,1,0.5), linewidth = 5, linestyle=:dot)
			CairoMakie.lines!(axs[i,j], xs, g.(xs),
				color=RGBA(0.5, 0.5, 0, 0.3), linewidth=20)
		end
	end
	fig
end
...
# ╔═╡ 310c94bf-1d6b-404d-ba23-1295865f6aee
md"""
 ↑あまりうまくいってないように見える。要確認。
"""

# ╔═╡ 9edcb4d1-e0ba-4071-8e3e-52862fafcedd
...
begin
	...
	X_sim = rand(0:35, N)
	KID = reduce(vcat, [fill(i, N_k[i]) for i in 1:4])

	a = rand(Normal(0, s_a), 4) .+ a0
	b = rand(Normal(0, s_b), 4) .+ b0
	d_sim = DataFrame(hcat(X_sim, KID), [:X, :KID])
	d_sim = @transform!(d_sim, :A = a[:KID],
		:B = b[:KID])
	
	d_sim = @transform!(d_sim, :Y = rand.(Normal.(:A .+ :B .* :X, s_Y)))
end

# ╔═╡ 97f43ad9-cd01-4fb6-90de-715febf45a5f
@model function simple_regression(X,Y)
	a ~ Normal(0, 100)
	b ~ Normal(0, 100)
	σ ~ truncated(Normal(0, 100), 0, Inf)

	for n in 1:size(X,1)
		Y[n] ~ Normal(a + b * X[n,:X], σ)
	end
end

# ╔═╡ f0d447e9-3ac6-4f8b-b69d-9418770ac57d
model_all = simple_regression(X, Y)

# ╔═╡ a44a6d03-e915-42d0-aa45-218bdf9945f4
chain_all = sample(model_all, NUTS(0.65), MCMCThreads(), 1_000, 3)

# ╔═╡ b4e2c2e4-bbc0-4b1f-871b-7d62b2416628
describe(chain_all)

# ╔═╡ 413f4da7-abf0-43fa-a16a-ac79f2ebcf14
StatsPlots.plot(chain_all)

# ╔═╡ 409760f3-3d54-47f3-b0a8-f222b32c7d70
@model function each_regression(X, Y)
	a ~ filldist(Normal(0, 100), 4)
	b ~ filldist(Normal(0, 100), 4)
	σ ~ truncated(Normal(0, 100), 0, Inf)

	for n in 1:size(X,1)
		Y[n] ~ Normal(a[X[n,:KID]] + b[X[n,:KID]] * X[n,:X], σ)
	end
end

# ╔═╡ ad8c91f8-4dfd-4943-9f16-319005b233e2
each_model = each_regression(X, Y)

# ╔═╡ 1ae51852-2797-479e-94f6-cd598d6a76ed
each_chain = sample(each_model, NUTS(0.65), MCMCThreads(), 1_000, 3)

# ╔═╡ c14b1c87-0119-4f95-bf6d-7a7faaaa8eb1
describe(each_chain)

# ╔═╡ 8430e904-f4f7-4c5e-8fd2-566b0f651be2
StatsPlots.plot(each_chain)
...
let
	...
	for i in 1:2
		for j in 1:2
			...
			for n in 1:size(each_chain[:,:,1], 1)
				a = each_chain[n, Symbol("a[$(k)]"), 1]
				b = each_chain[n, Symbol("b[$(k)]"), 1]
				f(x) = a + b * x
				CairoMakie.lines!(axs[i,j], xs, f.(xs), color=RGBA(0,1,1,0.02))
				CairoMakie.lines!(axs[i,j], xs, g.(xs),
					color=RGBA(0.5, 0.5, 0, 0.1), linewidth=10)
			end
		end
	end
	fig
...
let
	...
	for i in 1:2
		for j in 1:2
			...
			f(x) = a + b * x
			CairoMakie.lines!(axs[i,j], xs, f.(xs), 
				color=RGBA(0,1,1,0.5), linewidth = 3, linestyle=:dot)
		end
	end
	fig
end

# ╔═╡ 57ca4e28-efc6-4838-84ca-1e3889c71940
@model function hierarchical_model(X, Y)
	a0 ~ Normal(0, 100)
	b0 ~ Normal(0, 100)
	...
	a ~ filldist(Normal(a0, σa), 4)
	b ~ filldist(Normal(b0, σb), 4)

	μ = Vector(undef, size(X,1))
	for n in 1:size(X,1)
		μ[n] = a[X[n,:KID]] + b[X[n,:KID]] * X[n,:X]
		Y[n] ~ Normal(μ[n], σy)
	end
end

# ╔═╡ 170a47d9-3fe9-4b36-8b2b-ce541d8c1a9b
hie_model = hierarchical_model(X, Y)

# ╔═╡ e7a2fea8-d3f0-421a-abbe-668db5057fb8
hie_chain = sample(hie_model, NUTS(0.65),MCMCThreads(), 1_000, 3)

# ╔═╡ 29c80a27-ec3b-4283-b680-d978c4ac8739
describe(hie_chain)

# ╔═╡ 85d4976c-4665-40bf-a521-2aeeed592e75
StatsPlots.plot(hie_chain)
...
let
	...
	for i in 1:2
		for j in 1:2
			...
			a2 = quantile(each_chain[:, Symbol("a[$(k)]"), 1], 0.5)
...
# ╔═╡ c550894c-3a01-48ae-a7ae-8588a2f14f55
