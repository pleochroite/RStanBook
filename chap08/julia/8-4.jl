...
begin
	#_foo = d2.Weather
	#_weather_dummy = DataFrame((unique(_foo) .== permutedims(_foo))'[:,[2,3]], [:Weather_B, :Weather_C])
	#d2d = hcat(d2, _weather_dummy)
	#X2 = d2d[:,Not([:Weather, :Y])]

	function conv_weather(w)
		if w == "A"
			0
		elseif w == "B"
			0.2
...
let
	fig = Figure()
	ax = Axis(fig[1,1], xlabel="CourseID", ylabel="attendance rate")

	for w ∈ ["A", "B", "C"]
		_tmpdf = @subset(d2_agg, :Weather .== w)
		CairoMakie.scatter!(ax, _tmpdf.CourseID, _tmpdf.Y_mean)
	end
	fig
end
...
let
	keys_course = [Symbol("b_course[$(i)]") for i ∈ 1:10]
...
begin
	q10 = [quantile(qsm[:,i], 0.1) for i ∈ 1:size(qsm,2)]
	q50 = [median(qsm[:,i]) for i ∈ 1:size(qsm,2)]
...
let
	...
	CairoMakie.lines!(ax, fpr10, tpr10)
	CairoMakie.lines!(ax, fpr90, tpr90)
	
	CairoMakie.lines!(ax, [0,1], [0,1],linestyle=:dot, color=:gray50)
...
model_predict = DynamicPPL.Model{(:y,)}(:model_predict_missing_data, model_missing.f, model_missing.args, model_missing.defaults)
