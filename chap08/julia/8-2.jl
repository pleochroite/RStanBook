...
# ╔═╡ 010bc043-e655-4f41-bb4d-913bc9827d44
using Turing, MCMCChains, StatsPlots, CairoMakie, Distributions, CSV, DataFrames, DataFramesMeta

# ╔═╡ fb7b6ac4-54ff-45e7-90c5-7a37e0afd1dd
using GLM, LinearAlgebra

# ╔═╡ 97c52658-61b6-11ec-0204-0bc85d0e4f47
md"""
## 8.2 複数の階層を持つモデル
"""

# ╔═╡ ceaf0a61-80f8-4aa8-a8b6-7481fd40879b
set_theme!(Theme(Axis=(;xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 05d9fca5-69a1-4bb9-84ac-43fcd2607a5d
d = CSV.read("../input/data-salary-3.txt", DataFrame)

# ╔═╡ 1151f487-7338-431c-9e28-9ef8cbe22ee2
begin
	X = d[:,Not(:Y)]
	Y = d[:,:Y]
end

# ╔═╡ 18c9edca-af5e-4b4e-8463-33ebe84c09f6
md"""
### 8.2.1 解析の目的とデータの分布の確認
"""

# ╔═╡ 8d73c6b2-1b70-4a3f-82f1-edf23fed0c07
let
	...
	for g in unique(X.GID)
		_tmpdf = @subset(d, :GID .== g)
		CairoMakie.scatter!(ax, _tmpdf.X, _tmpdf.Y, label="$(g)", markersize=15)
	end
	...
	CairoMakie.lines!(ax, xs, f.(xs), linewidth=15, color=RGBA(0.3,0.3,0.3,0.5))
	
	axislegend("GID"; position=:lt)
	fig
end	
...
let
	...
	for (i, g) in enumerate(unique(X.GID))
		_tmpdf = @subset(d, :GID .== g)
		CairoMakie.scatter!(ax[i], _tmpdf.X, _tmpdf.Y, label="$(g)", markersize=15)
		
		m = glm(@formula(Y ~ X), _tmpdf, Normal(), IdentityLink())
		a = coef(m)[1]
		b = coef(m)[2]
		g(x) = a + b * x
...
# ╔═╡ 895a614f-4c9d-48f5-aad4-14b1baea18e7
md"""
### 8.2.4' Turingで実装
"""

# ╔═╡ 0ce46f7a-dcac-4dc3-a8aa-a5b8f65b347d
@model function hierarchies1(X, Y, K2G)
	a0 ~ Normal(0, 100)
	b0 ~ Normal(0, 100)

	σag ~ truncated(Normal(0, 100), 0, Inf)
	σbg ~ truncated(Normal(0, 100), 0, Inf)

	ag ~ filldist(Normal(a0, σag), 3)
	bg ~ filldist(Normal(b0, σbg), 3)

	σa ~ truncated(Normal(0, 100), 0, Inf)
	σb ~ truncated(Normal(0, 100), 0, Inf)
	σy ~ truncated(Normal(0, 100), 0, Inf)

	ak = Vector(undef, size(K2G, 1))
	bk = Vector(undef, size(K2G, 1))
...
model_hierarchies1 = hierarchies1(X, Y, unique(X[:,[:KID, :GID]]))

# ╔═╡ 26014805-e6c9-4d6d-a009-be31a815d30a
chain_hierarchies1 = sample(model_hierarchies1, NUTS(0.65), MCMCThreads(), 1_500, 4)
# takes ~450s for 1000 samples 

# ╔═╡ 83dd23b3-4d0c-4051-96a9-07cd9826436d
describe(chain_hierarchies1)
...
@model function hierarchies2(X, Y, K2G)
	...
	σa ~ filldist(truncated(Normal(0, 100), 0, Inf), 3)
	σb ~ filldist(truncated(Normal(0, 100), 0, Inf), 3)
	σy ~ filldist(truncated(Normal(0, 100), 0, Inf), 3)
...
model_hierarchies2 = hierarchies2(X, Y, unique(X[:,[:KID, :GID]]))

# ╔═╡ 331065bc-5de7-47ba-974e-b2141b4e7c72
chain_hierarchies2 = sample(model_hierarchies2, NUTS(0.65), MCMCThreads(), 2_000, 4)
# takes 600 seconds for 1＿０００　samples
# takes 1500 seconds for 2_000 samples, 4chain

# ╔═╡ 9869e19e-102f-48b0-8844-9d96f03aa756
describe(chain_hierarchies2)
