...
# ╔═╡ 3437c22c-6d81-11ec-0e81-d556ea5a04dd
using Turing, MCMCChains, Distributions, Bijectors, StatsPlots, CairoMakie, DataFrames, DataFramesMeta, CSV, Colors

# ╔═╡ b292250a-20cc-4429-913b-00aead6b0661
md"""
reference. https://huijzer.xyz/posts/latent/
"""

# ╔═╡ d6c506db-cbd1-4fc8-b171-166caef354c0
dat = CSV.read("../input/data-mix1.txt", DataFrame)

# ╔═╡ 7fe10be8-2e4e-471c-b9a4-e21b03cd9726
md"""
#### model with label switching
"""

# ╔═╡ df977811-0201-4321-8742-8a0ac8c6c04e
@model function ex_mixture(Y)
	w ~ Dirichlet(2, 1)
	μ ~ filldist(Normal(0, 100), 2)

	n = length(Y)
	Y ~ filldist(MixtureModel(Normal, μ, w), n)
end

# ╔═╡ ec427574-b921-410a-b561-4ec8ff54cd1f
model = ex_mixture(dat.Y)

# ╔═╡ 3dc387c1-c6db-48a1-9c85-74c53abf5b36
chain = sample(model, NUTS(0.65), MCMCThreads(), 1000, 3)

# ╔═╡ ee4b2172-714e-47ed-9dd2-549611eac89f
describe(chain)

# ╔═╡ 651df6e1-6a24-407a-83d9-134041bef904
StatsPlots.plot(chain)

# ╔═╡ d0e750a1-42da-4a7c-9491-d8631e233f56
md"""
### model with ordered prior
"""

# ╔═╡ 20538cfa-8153-4d39-9ae5-7a12baf895ce
ordered(d::Distribution) = transformed(d, Bijectors.OrderedBijector())

# ╔═╡ d36aa3aa-217b-468c-bb82-093906dada0b
inv_ordered(X::Vector) = inv(Bijectors.OrderedBijector())(X)

# ╔═╡ aeaa0495-d90b-4d4d-8668-3f550688f89c
M = inv_ordered([0, 0.01])

# ╔═╡ c03e8ba9-37e6-456c-9000-d0189c699b23
@model function ordered_mixture(Y)
	w ~ Dirichlet(2,1)
	μ ~ ordered(arraydist([Normal(m, 10) for m ∈ M]))

	n = length(Y)
	Y ~ filldist(MixtureModel(Normal, μ, w), n)
end

# ╔═╡ 674cd8e5-1b38-4249-9536-ee7baa6686d5
ordered_model = ordered_mixture(dat.Y)

# ╔═╡ 7c3fdc21-6893-489d-8274-1b3565b07baa
ordered_chain = sample(ordered_model, NUTS(0.65), MCMCThreads(), 1_500, 3)

# ╔═╡ 745e1f83-91bc-4018-ac98-20bb82447b94
describe(ordered_chain)

# ╔═╡ d6bbde52-27d2-43ea-9286-e3651b526fab
StatsPlots.plot(ordered_chain)

# ╔═╡ d0fe3699-0051-417a-8939-3ff979bd6f23
