...
# ╔═╡ 673a3b00-6e21-11ec-35ce-09a49e4d1076
using Turing, MCMCChains, Distributions, StatsPlots, CairoMakie, CSV, DataFrames, DataFramesMeta, Colors, Bijectors

# ╔═╡ 239fb46f-0a40-4ddf-9d3b-7bb5bdad0496
using StatsFuns

# ╔═╡ 4be361a8-8d75-48a7-b564-e2b48e14d63f
using LazyArrays

# ╔═╡ 8c270f13-52a8-4706-b45e-fccfb0588301
using StatsBase

# ╔═╡ ee845631-956d-44de-b136-fb56634465ff
set_theme!(Theme(Axis=(;xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 551f97cc-204b-4f1e-896d-f78b315f695a
d_raw = CSV.read("../input/data-category.txt", DataFrame)

# ╔═╡ 7aaada7c-9e96-4540-b48b-1043de64f169
d = @transform(d_raw, :Income = :Income / 911.0)

# ╔═╡ ad4f9950-d65a-46f7-a9de-dc67de6f637e
begin
	X = Matrix(d[:,Not(:Y)])
	Y = d.Y
end

# ╔═╡ 78e0f572-2092-4744-af3b-5d6cdffe0827
dist = Dirichlet(6, 0.8)

# ╔═╡ b239d82e-df5c-4503-85f3-4042335c3d5e
b = dist |> bijector

# ╔═╡ 0f93b2ab-c78a-4472-b541-3cb5f555c54c
binv = dist |> bijector |> inv

# ╔═╡ ccf0a939-7e50-4bc2-a118-cc30f1a0da9a
td = transformed(dist, binv)

# ╔═╡ 6cf304f6-e92b-44d9-bc4b-c84277cfd560
rand(td)

# ╔═╡ a9e3fb40-4ff0-4a9d-8ad9-82fb4fe64a53
@model function multi_logi(X, Y)
	n_obs = size(X,1)
	n_vars = size(X,2) + 1
	n_cats = length(unique(Y))

	# fix μ1 be zeros for identifiability
	b_raw ~ filldist(Normal(0, 1), n_vars, n_cats - 1)
	b = hcat(zeros(n_vars), b_raw)

	Xb = hcat(ones(n_obs), X)
	
	μ = Xb * b

	for n ∈ 1:n_obs
		Y[n] ~ Categorical(softmax(μ[n,:]))
	end
	
	return μ
end

# ╔═╡ 4750687b-a7e0-4f65-a60d-6a0e0bf98621
rand(Categorical(StatsFuns.softmax([10, 2, 3, 4, 5])))

# ╔═╡ 0bd9f445-ab36-4f0d-a0e6-6b612a93df8c
model = multi_logi(X, Y)

# ╔═╡ d7740af4-0945-4acb-890d-57a1c3edb161
prior_chain = sample(model, Prior(), 500)

# ╔═╡ 04c723a8-1b47-4dc0-8ce3-8a7e6fd7a64a
describe(prior_chain)

# ╔═╡ 88873992-0036-4b3e-a411-6bd4fa0e21f2
StatsPlots.plot(prior_chain)

# ╔═╡ 99139dd1-decc-4065-85a3-0abd11a7bfeb
chain = sample(model, NUTS(), MCMCThreads(), 1500, 4)

# ╔═╡ 61aacfa3-a9f8-4920-80fb-a4c235dd7848
μ = generated_quantities(model, chain)

# ╔═╡ 27c38f32-921e-43e6-b84c-94b1ab3e4bde
softmax(μ[1][1,:])

# ╔═╡ 1775f7f5-6461-4b5d-985d-81f1e69d82a1
describe(chain)

# ╔═╡ b4be1338-7e7a-43f9-9302-16f49fd8f539
StatsPlots.plot(chain)

# ╔═╡ 297dadd3-5be3-4c59-9d36-8d95ea3162ad
md"""
#### 結果から予測させる
"""

# ╔═╡ 71f5afb3-26b0-4bea-88b1-5c299500c624
missing_data = Vector{Missing}(missing, length(Y))

# ╔═╡ 3c8b9d23-ea5c-4f82-a59f-8efdd741a81e
model_missing = multi_logi(X, missing_data)

# ╔═╡ 1d710188-ece2-428e-8122-1956c16fdac1
posterior_predict = predict(model_missing, chain)

# ╔═╡ 919fb099-4bd1-4044-b552-75ccd073276d
