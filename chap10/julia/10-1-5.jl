...
# ╔═╡ 8e9de914-7216-11ec-14ea-2f60501cd4fe
using Turing, MCMCChains, Distributions, StatsPlots, Bijectors, CairoMakie, CSV, DataFrames, DataFramesMeta, Colors

# ╔═╡ 2fde159a-e18f-4b40-acf1-8440d237f0f6
using PlutoUI

# ╔═╡ 393f9fb8-712f-428f-9108-298c357f83a8
md"""
### 10-1-5 ウサギとカメ
"""

# ╔═╡ 7b4414df-27e8-49a2-ae0f-0d2a8e056de5
set_theme!(Theme(Axis=(; xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 723e8a59-4253-4f5f-95f0-1f26566878ce


# ╔═╡ e9559359-c391-49fc-98ba-e030d3669afd
d = CSV.read("../input/data-usagitokame.txt", DataFrame)

# ╔═╡ bcc89522-513e-40f2-81dd-7d24fb2b228b
md"""
Let strengths of tortoise and rabbit be $\mu [1], \mu [2]$ respectively. 
We model that performances in the game $g$ of each player $i$ are generated from $\mathcal{N}(\mu [i], \sigma[i]) (i = 1, 2)$, and the player with greater performance wins.

Having small data, we assume $\sigma [1] = \sigma [2] = \sigma$.

In order to parameters be identifiable, we set $\mu [1] = 0, \sigma = 1$.

After all, the mechanism be modelled as follows,

$$\begin{eqnarray}
	performance[g,1] & \sim & \mathcal{N}(\mu [Loser[g]], 1) & \quad g = 1, ..., G\\
	performance[g,2] & \sim & \mathcal{N}(\mu [Winner[g]], 1) & \quad g = 1, ..., G\\
	performance[g,1] & < & performance[g,2] & \quad  g = 1, ..., G \\
	\mu [1] &=& 0
\end{eqnarray}$$
"""

# ╔═╡ 728f41c0-17ba-4824-927b-b2dc065eb464
md"""
#### Simulation
"""

# ╔═╡ ec1e8ca6-bb5c-4893-8b79-86a0fcc67329
@bind μ PlutoUI.Slider(-1:0.1:2, default=1.0, show_value=true)

# ╔═╡ 835f3a79-9686-418b-9c01-df05193ea06d
begin
	μs = [0, μ]
	sim() = [rand(Normal(μ,1)) for μ ∈ μs]
	winners = [findmax(sim())[2] for _ ∈ 1:30]
	count(i -> i == 1, winners)
end

# ╔═╡ ea95e9c4-713d-4f78-95fc-d630c63dda00
md"""
#### Modelling
"""

# ╔═╡ bee02fb6-201c-41b9-b2e1-4e9afbcb2f75
inv_ordered(x::Vector) = inv(Bijectors.OrderedBijector())(x)

# ╔═╡ badc4a44-9c93-4d7e-adf3-179e4d8e33ec
M = inv_ordered([0, 2.5])

# ╔═╡ 22b78bc5-abff-441d-a384-70e5b4b55c4a
hcat(rand.([Bijectors.ordered(arraydist([Cauchy(μ, 1) for μ ∈ M])) for _ in 1:10])...)'

# ╔═╡ 2c90dc26-1713-42e0-ac59-5bd3229df78c
rand(filldist(Normal(0, 1), 3))

# ╔═╡ 8f13e9fb-cbf8-4814-866d-c016c9b52805
@model function tortoise_rabbit(result)
	G = size(result, 1)

	b ~ Normal(0, 100)
	μ = [b, 0]

	M = inv_ordered([0.0, 2.5])
	#pf = Vector(undef, G)
	#for g ∈ 1:G
	#	pf[g] ~ Bijectors.ordered(arraydist([Normal(μ, 0.5) for μ ∈ M]))
	#	for i ∈ [1,2]
	#		pf[g][i] ~ Normal(μ[result[g,i]], 1)
	#	end
	#end
	pf ~ filldist(Bijectors.ordered(arraydist([Normal(μ, 100) for μ ∈ M])), G)

	#pf = Matrix(undef, G, 2)
	for g ∈ 1:G, (i, p) ∈ enumerate([:Loser, :Winner])
		pf[i,g] ~ Normal(μ[result[g,p]],1)
	end
end

# ╔═╡ 91d026d5-35ab-4939-b739-fa88d86f763c
m = tortoise_rabbit(d)

# ╔═╡ 34c03a7d-8459-499e-9bdb-1df4ab4bf2a3
chn = sample(m, PG(10), 500)

# ╔═╡ 7b8d1ded-3687-44da-a50d-aedfb0faefa3
describe(chn)

# ╔═╡ 62b9724d-9e3c-4f04-90c6-7d9ad01a8140
StatsPlots.plot(chn[:,[:b],:])

# ╔═╡ 31e8ee70-5048-403f-ae24-333c6411bc5a
