...
# ╔═╡ b093980e-608a-11ec-1b7d-59a588b2f234
using Turing, Distributions, MCMCChains, CairoMakie, StatsPlots, CSV, DataFrames, DataFramesMeta

# ╔═╡ cb54954b-91d5-4a8f-9f18-5182beaae9a2
md"""
## 7.7 説明変数にノイズを含む
"""

# ╔═╡ 08d914a0-773b-4aa7-aff5-8255618da52e
set_theme!(Theme(Axis=(xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 197a9b72-d6fc-4ccb-ba42-283f7e987182
d = CSV.read("../../chap04/input/data-salary.txt", DataFrame)

# ╔═╡ c27b15f0-c206-45c5-bd97-d06d2a56355f
X = d.X

# ╔═╡ 595c7254-d124-4041-9695-890c91e0b271
Y = d.Y

# ╔═╡ f892490d-b1d0-4d0b-8673-6e9316cb84b3
@model function ind_var_noise(X,Y)
	a ~ Normal(0, 100)
	b ~ Normal(0, 100)
	σ ~ truncated(Normal(0,100),0,Inf)
	
	Xspec = Vector(undef, size(X,1))
	μ = Vector(undef, size(X,1))
	for n in 1:size(X,1)
		Xspec[n] ~ Normal(X[n], 2.5)
		μ[n] = a + b * Xspec[n]
		Y[n] ~ Normal(μ[n], σ)
	end
end

# ╔═╡ 15592239-709d-47c8-ae59-b6c34f852e1f
model = ind_var_noise(X, Y)

# ╔═╡ a5e7de4d-3250-40ec-bc56-a1b6ec4c98bd
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_000, 4)

# ╔═╡ 787cc814-3d4d-43dc-9c11-4091564092b3
StatsPlots.plot(chain[:,[:a, :b],:])
