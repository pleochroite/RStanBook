...
# ╔═╡ d288ccc6-608d-11ec-15ac-651347a9b68b
using Turing, Distributions, MCMCChains, CairoMakie, StatsPlots, CSV, DataFrames, DataFramesMeta

# ╔═╡ 9cd6e9e3-c270-4f02-80f0-ca26cdb26750
md"""
## 7.9 外れ値
"""

# ╔═╡ a491ad30-8dfa-43b3-a222-56f5c62c949a
set_theme!(Theme(Axis=(xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ b41d4a87-c2ff-4c88-b7e8-2e735f0689fd
d = CSV.read("../input/data-outlier.txt", DataFrame)

# ╔═╡ 2fc602b4-69e8-4bb4-9b02-5e86150d22f7
CairoMakie.scatter(d.X, d.Y)

# ╔═╡ e8422c0c-5423-420d-8ded-02dabaf40e43
@model function cauchy(X,Y)
	a ~ Normal(0, 100)
	b ~ Normal(0, 100)
...
# ╔═╡ 04703c6e-e788-49fa-9b5c-8338f01c2f94
model = cauchy(d.X, d.Y)

# ╔═╡ 1c8f556f-f2f7-4fc0-b7e2-a6bfaabcf7e1
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_000, 3)

# ╔═╡ 1bc21529-3c2b-46f4-99c7-7ebcd892e825
describe(chain)

# ╔═╡ 8380c5ef-3173-42a2-ab24-d8f59a9650bd
StatsPlots.plot(chain)

# ╔═╡ 56392509-6c67-436e-b0fc-330ec7140972
...
# ╔═╡ ed8c8017-bd34-4f0b-ba80-617e9785bd23
modelB = normal(d.X, d.Y)

# ╔═╡ f9d88111-628c-41f6-b286-2cf5583f1f42
chainB = sample(modelB, NUTS(0.65), MCMCThreads(), 1_000, 3)

# ╔═╡ 2be15a2a-79e0-4ef4-b257-af5d3953e2e2


# ╔═╡ 6126c99b-2dfc-42a2-a9be-a8db1631ad4d
let
	fig = Figure()
	ax = Axis(fig[1,1], xlabel="X", ylabel="Y")

	xs = range(0, 12, length=60)
	
	for i in 1:size(chainB[:,:a,1], 1)
		a = chainB[i,:a,1]
		b = chainB[i,:b,1]
		g(x) = a + b * x
		CairoMakie.lines!(ax, xs, g.(xs), color=RGBA(0.5,0.5,0.5,0.02))
	end
	...
	for i in 1:size(chain[:,:a,1], 1)
		...
		b = chain[i,:b,1]
		f(x) = a + b * x
		CairoMakie.lines!(ax, xs, f.(xs), color=RGBA(0,0.5,0,0.02))
	end
...
# ╔═╡ 2e8c92be-deb5-45ea-9a5e-ed854ec49507
