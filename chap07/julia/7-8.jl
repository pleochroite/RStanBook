...
# ╔═╡ e592c554-30b5-47e8-ba63-a2b89a5d4c86
_X = rand(200)

# ╔═╡ fabd7f45-23aa-42d9-8222-31f1b26af065
_Y = rand.(Normal.(1 .+ _X, 0.25))

# ╔═╡ 3bfde522-112a-4d80-b24e-8b5d270ad695
_Ycensored = map(x -> x < 1.2 ? 1.2 : x, _Y)

# ╔═╡ c900bd9d-8a5b-42df-b4cc-835ebba81b8b
d_raw = DataFrame(hcat(_X, _Ycensored), [:X, :Y])

# ╔═╡ 58850e88-e7e4-462a-9425-8f45d25a2c0b
CairoMakie.scatter(d_raw.X, d_raw.Y)

# ╔═╡ 0c64bb04-002c-4f3a-a531-01854006b3c6
d = @transform(d_raw, :censored = :Y .== 1.2)

# ╔═╡ 50ffee71-19eb-40e7-b82e-b24085adfab4
begin
	X = d[:,Not(:Y)]
	Y = d[:,:Y]
end

# ╔═╡ 1c9b961a-a59e-49ad-8af7-d2f1778c9551
md"""
#### モデル関数作成
"""

# ╔═╡ 7c4e7ee0-e262-4219-9978-e9a5eaa23ecb
@model function censored_regression(X, Y)
	a ~ Normal(0, 10)
	b ~ Normal(0, 10)
	...
	for n in 1:size(X,1)
		μ[n] = a + b * X[n,:X]
		if X[n, :censored] # left censored
			Turing.@addlogprob! logcdf(Normal(μ[n], σ), Y[n])
		# elseif # right censored
			# Turing.@addlogprob! logccdf(Normal(μ[n], σ), Y[n])
		else # observed
			Turing.@addlogprob! logpdf(Normal(μ[n], σ), Y[n])
		end
	end
end

# ╔═╡ 69c7fab5-617b-43d6-ba3f-b739238e9406
model = censored_regression(X, Y)

# ╔═╡ 44aba0f0-fe3d-472f-90aa-c647a2c2e869
md"""
#### サンプリング
"""

# ╔═╡ 08205dec-2a5c-4170-a716-4500f2d16bfe
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_000, 3)

# ╔═╡ 7f15819b-5bf9-4d66-b3bd-94aeec7deb8c
describe(chain)

# ╔═╡ 57c13de0-0340-4d03-b303-7a610f60139f
StatsPlots.plot(chain)

# ╔═╡ ed54af9c-3b8c-4df6-93b3-dadff09b04e8
md"""
### 打ち切りを考慮しないモデルと比較
"""

# ╔═╡ 39f32b21-f324-4668-9b92-c62528660f06
...
# ╔═╡ b60044a8-b85d-434c-939f-676a8c418911
modelB = linear_regression(X, Y)

# ╔═╡ 03ff4549-19e4-4031-9d0c-ae3ae829267d
chainB = sample(modelB, NUTS(0.65), MCMCThreads(), 1_000, 3)

# ╔═╡ 7249f77d-32e3-496a-8da1-54b94f541a94
describe(chainB)

# ╔═╡ f3c808b7-40e6-4f9c-864f-7ebcfd8f2d0d
StatsPlots.plot(chainB)

# ╔═╡ 5aa1582b-2324-489b-8ba4-871d4cc9b415
let
	fig = Figure()
	...
	for i in 1:size(chainB[:,:a,1], 1)
		a = chainB[i,:a,1]
		b = chainB[i,:b,1]
		g(x) = a + b * x
		CairoMakie.lines!(ax, xs, g.(xs), color=RGBA(0.5, 0.5, 0.5, 0.01))
	end

	CairoMakie.scatter!(ax, X.X, Y)
