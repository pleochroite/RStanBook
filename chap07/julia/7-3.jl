...
# ╔═╡ 14e68caa-7f3a-40eb-9b53-906901192b70
set_theme!(Theme(Axis=(xgridstyle=:dot, ygridstyle=:dot, xgridcolor=:gray30, ygridcolor=:gray30)))

# ╔═╡ 8f581e4a-123c-4dff-8cca-0243ca005682
d = CSV.read("../input/data-aircon.txt", DataFrame)

# ╔═╡ 6c616ec3-d50e-4a2f-abd2-1f59ed7d1de5
CairoMakie.scatter(d.X, d.Y)

# ╔═╡ 53d12b32-2dab-4f93-8228-8444c540456c
@model function regression_square(X,Y)
	a ~ Normal(0, 100)
	b ~ Normal(0, 100)
	x0 ~ truncated(Normal(15, 100), 0, 30)
...
# ╔═╡ 0f417bfe-6afd-4afb-85d4-fbddd7dd8048
model = regression_square(d[:,:X], d[:,:Y])

# ╔═╡ eb0d5fc7-68e8-4e7f-8407-5f32b2a1d0b9
chain = sample(model, NUTS(0.65), MCMCThreads(), 1_500, 4)

# ╔═╡ b183678a-cc39-4e2f-ae2d-5f6dbf3ac065
describe(chain)

# ╔═╡ f24ae418-8743-46f6-84cf-7aa36cee4a7e
StatsPlots.plot(chain)

# ╔═╡ 23aba522-e359-4be7-9fa0-05218c0cde55
μs = generated_quantities(model, chain)[1]

# ╔═╡ 05b606cb-6d67-4f6c-8909-f40b7c0a3a6d
let
	fig = Figure()
	ax = Axis(fig[1,1], xlabel="X", ylabel="Y")

	xs = range(-3, 32, length=70)
	for i in 1:size(chain[:,:,1], 1)
		a = chain[i,:a,1]
		b = chain[i,:b,1]
		x0 = chain[i,:x0,1]
		f(x) = a + b * (x - x0)^2
...
# ╔═╡ e1414254-091d-4862-a7e0-e5389b8611de
md"""
### 時系列データの例
"""

# ╔═╡ a44a6c6e-6fd9-4653-b393-fbe5176e2e57
dB = CSV.read("../input/data-conc.txt", DataFrame)

# ╔═╡ c48455bc-4adf-4d51-af29-3de946a285ee
CairoMakie.scatterlines(dB.Time, dB.Y)

# ╔═╡ 40205339-247d-46ce-8ea1-e973ceb9e6c6
@model function decay_model(X,Y)
...
# ╔═╡ 633233e6-b7b9-45fe-bc3a-0c5b0876bd1c
modelB = decay_model(dB.Time, dB.Y)

# ╔═╡ 0d6cddc7-591b-4b6c-a8d5-d4b620dc5bad
chainB = sample(modelB, NUTS(0.65),MCMCThreads(), 1_000, 4)

# ╔═╡ 5b339460-503d-4bee-8ff4-e66a045ed640
describe(chainB)

# ╔═╡ 3e0ddabb-2ad3-45bc-91ff-0b8f9f950c62
StatsPlots.plot(chainB)

# ╔═╡ 7a1aa043-5aec-463b-80cd-a5e247122d18
let
	fig = Figure()
	ax = Axis(fig[1,1], xlabel="Time", ylabel="Y")

	xs = range(0, 25, length=50)
	
	for i in 1:size(chainB[:,:,1], 1)
		a = chainB[i,:a,1]
		b = chainB[i,:b,1]
		f(x) = a * (1 - exp(-b * x))
		lines!(ax, xs, f.(xs), color=RGBA(0,0.5,0,0.01))
	end

	CairoMakie.scatterlines!(ax, dB.Time, dB.Y)
	fig
end
