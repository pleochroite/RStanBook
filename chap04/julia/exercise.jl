...
# ╔═╡ 5eb6219c-5fb7-11ec-31f5-e307a7ffad56
using Turing, Distributions, MCMCChains, CairoMakie, StatsPlots, DataFrames, DataFramesMeta


# ╔═╡ 7c0bfe29-e4c5-4c4c-aa74-07080de538a1
begin
	N1 = 30
	N2 = 20
	Y1 = rand(Normal(0, 5), N1)
	Y2 = rand(Normal(1, 4), N2)
	X = vcat(repeat([1], 30), repeat([2], 20))
	Y = vcat(Y1, Y2)
	N = size(X, 1)
end

# ╔═╡ 46bce901-f21d-4787-8f11-b55ebff68e45
md"""
(1) 各グループの値に差が認められるか、大凡把握するために図を描く。
"""

# ╔═╡ 44b1005b-639e-4844-a4c6-022723753d9d
CairoMakie.boxplot(X, Y)

# ╔═╡ 07c92fdc-dd9c-47dc-aca2-2d34934bd5fc
md"""
(3) モデルファイルを作成してMCMCを実行。
"""

# ╔═╡ 08e223a0-728c-494a-89eb-3d91a4252f5f
@model function t_test(Y1, Y2)
	μ1 ~ Normal(0, 100)
	μ2 ~ Normal(0, 100)
...
# ╔═╡ 8e4ae732-34bf-41fb-9ae6-596c24ecee2d
model_tt = t_test(Y1, Y2)

# ╔═╡ 4ed2a0dc-61d3-49f0-8d22-938cbe9c6649
chn_tt = sample(model_tt, NUTS(0.5), MCMCThreads(), 1_000, 4)

# ╔═╡ ad1d92f5-c123-4709-bef5-708102843c58
describe(chn_tt)

# ╔═╡ 7c390abf-8867-4de4-943b-a3e9177f3177
StatsPlots.plot(chn_tt)

# ╔═╡ d6b73e0f-cca6-495a-82a3-9eefc2c6a2c7
md"""
(4) 得られたMCMCサンプルからProb[μ1 < μ2]を計算
"""

# ╔═╡ c51c2405-6c71-432a-b649-8326b370033e
count(chn_tt[:,:μ1,1] .< chn_tt[:,:μ2,1]) / 1000

# ╔═╡ f4cc5884-a8cf-4290-ab27-75bee1c301dd
md"""
(5) グループごとの標準偏差が異なる場合のモデルを考える（Welchの$t$検定に相当）。そして同様にProb[μ1 < μ2]を計算。
"""

# ╔═╡ 244b730e-4f8c-4c79-bdaa-8f62956f4467
@model function welch_t_test(Y1, Y2)
	μ1 ~ Normal(0, 100)
	μ2 ~ Normal(0, 100)
...
# ╔═╡ c69c56b4-2c3d-449d-a879-e8f476da2acc
welch_model = welch_t_test(Y1, Y2)

# ╔═╡ b67cf385-36fc-43fa-8d68-5c1ef9cf7f1e
chn_welch = sample(welch_model, NUTS(0.5), MCMCThreads(), 1000, 4)

# ╔═╡ 5f6acb8a-ec48-4dab-a698-f8dbe64e7043
describe(chn_welch)

# ╔═╡ c25c2c72-8a78-42fc-8690-470015b66184
StatsPlots.plot(chn_welch)

# ╔═╡ 73bd4b0d-188c-4d62-ba11-70c7a615fe4b
count(chn_welch[:,:μ1,1] .< chn_welch[:,:μ2, 1]) / 1000
