...
# ╔═╡ 1178b59a-5f4c-11ec-0a29-2121aff9d684
using Turing, Distributions, MCMCChains,StatsPlots,CSV, DataFrames

# ╔═╡ 07397acc-e7c8-4dce-8e67-fa1d0f61355e
using CairoMakie

# ╔═╡ 6fa0bbab-0a80-4a83-947b-78a1d0a09ccf
md"""
## 4.4 単回帰
"""

# ╔═╡ f5307f8c-dcf6-4f1b-9b90-952b828616e8


# ╔═╡ 7136ea75-19c2-47b7-b355-8c7ddd2a0396
set_theme!(Theme(Axis = (; xgridcolor=:gray30, ygridcolor=:gray30,
	xgridstyle=:dot, ygridstyle=:dot)))

# ╔═╡ aaa39e68-0e87-4ef5-8674-60697ed55e22
md"""
### 前処理
...
# ╔═╡ c40022d7-89b9-45d9-949b-9bb10e622f59
d = CSV.read("../input/data-salary.txt", DataFrame)

# ╔═╡ 549f302f-7cc7-4b97-80c3-875ef3d61a3b
md"""
### 4.4.2 データの分布の確認
"""

# ╔═╡ 6eff3b55-dbe4-4e57-8b24-1505660d200d
let
	fig = Figure()
	ax = Axis(fig[1,1], xlabel="X", ylabel="Y")
	CairoMakie.scatter!(ax, d.X, d.Y)
	fig
end

# ╔═╡ 76d907ef-4d88-4e75-9192-fa2c2e020527
X = d[:,:X]

# ╔═╡ 3adff50e-d53e-42a4-88bc-a478dcb3c52c
Y = d[:,:Y]

# ╔═╡ f2a835cf-1898-4bb6-845c-c52b5ed6d312
md"""
### 4.4.5' Turingで実装
"""

# ╔═╡ 70832aa5-6b09-4020-9c70-4816f3a8d0ef
...
# ╔═╡ 5a38e50b-0267-40c3-913f-3b6acfa59c58
model = linear_regression(X, Y)

# ╔═╡ 86404100-9907-414d-9eb3-f5392804e694
md"""
### 4.4.6 モデルの実行
"""

# ╔═╡ dd136dcc-d151-455f-8ef5-6aaf80254608
chain = sample(model, NUTS(0.65), MCMCThreads(), 1000, 3)

# ╔═╡ 1a8450c4-aa0f-408e-927a-fe816e1878b0
md"""
### 4.4.7 結果の見方

平均や分散、各パーセンタイル点を見るには`describe`を使う。
"""

# ╔═╡ abbfa9c4-4e48-4fdc-a71d-1ce8ac40522c
describe(chain)

# ╔═╡ 89a2f7db-9cb1-47f5-a9e1-012bd8c4eb91
md"""
トレースプロットとパラメタの事後分布を見るには`StatsPlots.plot`を使う。
"""

# ╔═╡ 5f31a149-293f-4f2e-b77e-2680cc55d9d3


# ╔═╡ 9ef552f6-d1f8-40d0-8174-22c785fe09ec
StatsPlots.plot(chain)

# ╔═╡ a69158c5-45e0-49a9-8afc-417b45e8bd6f
md"""
### 4.4.11 ベイズ信頼区間とベイズ予測区間の算出

MCMCサンプルの分布を概観するには`corner`を使うと良さげ。
"""

# ╔═╡ e2bab255-5dbc-4e7d-a89c-75beef8013e2


# ╔═╡ 15c938e6-6365-4c28-ade6-4c2b800bbb14
corner(chain)

# ╔═╡ 96ac4626-fda6-432a-a6ac-e162a191a737
md"""
観測データとMCMCサンプルをプロットして当てはまりを見る。
"""

# ╔═╡ b5565efe-d267-432f-9221-29ccff0209d0
let
	fig = Figure()
	ax = Axis(fig[1,1]; xlabel="X", ylabel="Y")

	xs = range(20,60,length=100)
	
	for i in 1:size(chain[:,:a,1],1)
		a = chain[i,:a,1]
		b = chain[i,:b,1]
		f(x) = a + b*x
		CairoMakie.lines!(ax, xs, f.(xs), color=RGBA(0,0.5,0, 0.01))
	end
...
# ╔═╡ 1e048f81-81a0-479e-979c-46f4668a4a0f
md"""
50%信頼区間や95%信頼区間をプロット。Makie.jlで帯状に塗る方法はあるのだろうか
"""

# ╔═╡ cd34b6b4-aae6-46eb-a7fe-4faacafbfed8
coefs = DataFrame([quantile(chain[:,key,1], [0.025, 0.25, 0.50, 0.75, 0.975]) for key in [:a, :b]], [:a, :b])

# ╔═╡ dff4376e-8eb6-466a-aaba-d6f456060379


# ╔═╡ dd0795ef-5c8a-45a4-a211-7ea036504dda
let
	fig = Figure()
	ax = Axis(fig[1,1]; xlabel="X", ylabel="Y")
	...
	for (i, color) in enumerate([:pink, :orange, :black, :orange, :pink])
		a = coefs[i,:a]
		b = coefs[i,:b]
		f(x) = a + b * x
		CairoMakie.lines!(ax, xs, f.(xs), color=color)
	end
...
# ╔═╡ 10089274-212c-4c92-af19-51372df381cd
md"""
### 4.4.12 transformed parametersブロックとgenerated quantitiesブロック

Turingは、Stanのtransformed parametersやgenerated quantitiesで可能なことのすべてができるわけではないようだ([参考](https://discourse.julialang.org/t/turing-jl-update-variables-within-the-model/35168))。
"""

# ╔═╡ c2f32c9f-041c-44d9-8b4b-89aa0c2713d0
@model function linear_regression2(X,Y)
	...
	y_base = Vector(undef, N)
	for n in 1:N
		y_base[n] = a + b * X[n]
		Y[n] ~ Normal(y_base[n], σ)
	end
end		

# ╔═╡ 8f4d6f38-7f48-4182-b34a-f80c7c733b9a
